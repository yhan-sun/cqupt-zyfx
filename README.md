...
- `/club.html`：赛事与足迹，自邮飞翔公众号公开记录与校园赛事档案
- `/gallery.html`：跑步影像，公众号跑团影像、协会补充影像与校园赛事照片
- `/science.html`：科学跑步专区
...
最终构建顺序：基础静态页 → 公众号档案 → QQ 加入体验 → 信息架构 → 自邮飞翔文化叙事 → 独立协会介绍与补充影像 → 动效/大图体验 → 来源与导航收尾。不要直接编辑 `dist`。
...
- `scripts/association-build.mjs`：独立协会页、首页协会入口、赛事页补充影像与影像页扩展。
- `scripts/association-finalize.mjs`：协会导航 active-state、公开文案与资料来源收尾。
