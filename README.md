...
正式构建会处理三类外部媒体：14 项校园/赛事图片、16 个科学跑步动作媒体、8 幅公众号跑团资料图。外部媒体按登记信息验证后同站托管；动作与公众号素材还校验固定哈希。跑团提供的 QQ 二维码可解析为 `https://qm.qq.com/q/9rKOuWR8Ag`，站内使用由同一链接重新生成的 `assets/join-qq.svg`，避免外部图片热链和二次 JPEG 压缩损坏。
...
Actions 覆盖单元/内容检查、图片处理、桌面和手机浏览器、键盘、无 JavaScript、静态直达路由、二维码本地加载、QQ 深链接、QQ群号文字备用以及历史/当前信息边界。构建只读，只有 main 部署任务拥有 `pages: write` 和 `id-token: write`。
...
| `data/join.json` | 当前 QQ 群号、QQ 加群链接、参与说明、核对日期和活动类型 |
| `assets/join-qq.svg` | 从跑团提供二维码解析出的同一 QQ 邀请链接重新生成的站内二维码 |
...
- QQ 群：`468686951`
- QQ 加群入口：`https://qm.qq.com/q/9rKOuWR8Ag`
- 群名：重邮约跑团
- 定位：为重邮学子搭建的跑步交流平台
...
学校标识只用于识别所属学校，不宣称学校认证。用户提供的 QQ 二维码和由其解析出的 QQ 邀请链接只用于当前跑团交流入口，不把其中信息扩展成学校行政服务或固定活动承诺。
