...
const { associationPages } = await import('./association-build.mjs');
...
const builtPages = [...pages.keys(), ...officialPages, ...joinPages, ...structurePages, ...associationPages];
const locations = builtPages.filter((file, index, all) => file !== '404.html' && all.indexOf(file) === index).map(file => `<url><loc>${origin}${file === 'index.html' ? '' : file}</loc></url>`).join('');
...
console.log(`Built ${new Set(builtPages).size} pages with a standalone member-sourced association story, 3 association-provided photographs, 20 self-flying official-account photographs, culture-led event archive, standalone running gallery, motion-safe reveals, click-to-enlarge running photography, science hub and verified QQ join experience${offline ? ' from the offline source cache' : ''}.`);
