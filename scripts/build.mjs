...
const runPython = (script, label, extra = []) => {
  const result = spawnSync(process.env.PYTHON || 'python3', [path.join(root, 'scripts', script), ...extra], { stdio: 'inherit', cwd: root });
  if (result.error) throw result.error;
  if (result.status !== 0) throw new Error(`${label} failed (${result.status})`);
};
runPython('media.py', 'Media processing', offline ? ['--offline'] : []);
runPython('science-media.py', 'Movement processing', offline ? ['--offline'] : []);
runPython('official-media.py', 'Official media processing', offline ? ['--offline'] : []);
...
console.log(`Built ${new Set(allPages).size} pages with 20 official-account photographs, 9 association-provided photographs, standalone association profile, culture-led event archive, expanded running gallery, motion-safe reveals, science hub and verified QQ join experience${offline ? ' from the offline source cache' : ''}.`);
