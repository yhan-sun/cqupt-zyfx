...
with sync_playwright() as p:
    ...
    qr = page.locator('.home-join-band img[data-join-qr]')
    qr.wait_for(state='visible')
    qr.evaluate('(img)=>img.decode()')
    check('join home: QR is local and decoded', qr.evaluate("img=>img.naturalWidth>0 && new URL(img.src).origin===location.origin && img.src.endsWith('/assets/join-qq.jpg')"))
    check('join home: no remote runtime QR request', not any('qrcode' in req and not req.startswith(url) for req in requests))
    ...
    check('join page: group number has multiple text fallbacks', page.locator('text=468686951').count() >= 3)
    ...
    check('join page: no fake registration form exists', page.locator('form').count() == 0)
    ...
    qr = page.locator('.join-hero img[data-join-qr]')
    qr.evaluate('(img)=>img.decode()')
    check('join page: supplied QR decodes as a browser image', qr.evaluate('img=>img.complete&&img.naturalWidth===560&&img.naturalHeight===560'))
    page.locator('.join-hero [data-copy-group]').click()
    check('join page: clipboard interaction is keyboard-focusable', page.locator('.join-hero [data-copy-group]').evaluate('(el)=>el.tagName==="BUTTON"'))
