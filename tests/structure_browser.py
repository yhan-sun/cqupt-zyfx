...
with sync_playwright() as p:
    ...
    check('home: association now points to standalone profile',page.locator('#main-nav a',has_text='协会介绍').get_attribute('href')=='about.html')
    ...
    page.goto(url+'club.html',wait_until='networkidle');wait_images(page,'img[data-official-image],img[data-association-image],.culture-campus-card img')
    ...
    check('events: association supplement adds supplied imagery',page.locator('.association-club-supplement img[data-association-image]').count()==3)
    ...
    check('gallery: archive now has thirty-five photos',page.locator('[data-gallery-page-item]').count()==35)
    check('gallery: nine association-provided photos are separated',page.locator('[data-gallery-page-item][data-category="association"]').count()==9)
    check('gallery: twenty official-account photos remain',page.locator('img[data-official-image]').count()>=20)
    ...
    check('gallery filter: association reveals nine supplied photos',page.locator('[data-gallery-page-item]:visible').count()==9)
    ...
    check('sources: all official-account media still carry explicit non-open-license wording','未发现开放许可' in page.locator('#official-wechat').inner_text())
    ...
    page.goto(url+'club.html',wait_until='networkidle');page.set_viewport_size({'width':390,'height':844});wait_images(page,'img[data-official-image],img[data-association-image]')
    ...
    check('no JS: all thirty-five photos remain readable',fallback.locator('[data-gallery-page-item]:visible').count()==35)
    ...
    check('no JS: all eight club records, supplement and three campus events remain readable',fallback.locator('[data-official-post]').count()==8 and fallback.locator('.association-club-supplement').count()==1 and fallback.locator('.culture-campus-card').count()==3)
