...
test('association: supplied images keep a readable source size',async()=>{for(const x of data.items){const metadata=await sharp(fileURLToPath(new URL(`../assets/member/${x.file}`,import.meta.url))).metadata();assert.equal(metadata.width,x.width);assert.equal(metadata.height,x.height);assert.ok(Math.max(metadata.width,metadata.height)>=1200,`${x.id} is too small`)}});
