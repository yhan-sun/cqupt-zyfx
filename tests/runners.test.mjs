...
test('runners: profile data keeps the supplied fields and registered image', () => {
  assert.equal(runners.items.length, 3);
...
test('runners: page is rendered from the shared model and preserves provenance', () => {
  ...
  assert.ok(html);
  for (const text of ['跑友风采', '胡钢', '2025级', '自动化学院', '17:24', '36:35', '1:20:57', '日拱一卒，功不唐捐', '俞沛君', '2023级', '生命健康信息学院', '17:59', '1:31:52', '3:48:00', '繁霜尽是心头血，洒向千峰秋叶丹', '杨昊贤', '国际学院', '17:30', '1:21:30', '2:59:47', '我是奶龙！我就是奶龙！！！']) {
