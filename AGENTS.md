...
The current QQ join source lives in data/join.json and assets/join-qq.svg. The user-supplied QR resolves to `https://qm.qq.com/q/9rKOuWR8Ag`; the site SVG is regenerated from that exact target to avoid compression damage. Group number, direct QQ link, QR, group name, audience wording or participation description changes require re-verification. Do not resurrect old WeChat recruitment deadlines, old QR codes, historical achievement thresholds or past weekly training times as current information. The site must keep both a text group-number fallback and a direct QQ join action, and must not create a fake registration-success flow.
